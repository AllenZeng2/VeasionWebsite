...
<script type="text/javascript">
	...
	function loadData(parms){
		grid = $("#maingrid4").ligerGrid({
			url:"${pageContext.request.contextPath}/admin/desktop/url/urlSearch",
			columns : [ {
				display : 'id',
				name : 'id',
				width : 35
	...
	function add(){
		window.parent.window.f_addTab("addUrl", "新增Url", "${pageContext.request.contextPath}/admin/desktop/url/goUrlModify");
	}
	
	function update(id, name){
		window.parent.window.f_addTab("updateUrl", name, "${pageContext.request.contextPath}/admin/desktop/url/goUrlModify?id="+id);
	}
	
	function del(id, name){
		if(confirm("确定要删除“"+name+"”?")){
			$.ajax({
				url:"${pageContext.request.contextPath}/admin/desktop/url/urlDelete?id="+id,
				type:"post",
